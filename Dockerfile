...
RUN if [ "${USE_HOST_SDK}" != "true" ]; then \
      ...
      echo "Using host-mounted SDK at $ANDROID_SDK_ROOT; skipping cmdline-tools download"; \
    fi
...
RUN if [ "${USE_HOST_SDK}" != "true" ]; then \
      ...
      echo "Using host-mounted SDK; skipping sdkmanager installs"; \
    fi
...
# Build-time AVD creation is optional; when mounting host SDK, defer to runtime script
RUN if [ "${USE_HOST_SDK}" != "true" ]; then \
      echo "no" | avdmanager --verbose create avd --force --name "${EMULATOR_NAME}" --device "${EMULATOR_DEVICE}" --package "${EMULATOR_PACKAGE}"; \
    else \
      echo "Using host-mounted SDK; skipping AVD creation at build time"; \
    fi
